    ...
    const saveInspections = async () => {
      const inspectionsBase64 = await Promise.all(
        inspections.map(async (i) => {
          const baseline = i.baselineImage instanceof File ? await fileToBase64(i.baselineImage) : i.baselineImage;
          const maintenance = i.maintenanceImage instanceof File ? await fileToBase64(i.maintenanceImage) : i.maintenanceImage;
